...
## 当铺 / 商城（`lib/shop.ts` 是唯一真相）

**房间 = 一套装扮（loadout）**，每个槽位一件。新账号从地下室起步（掉漆木桌 + 裸灯泡 +
折叠铁椅 + 铁皮烟灰缸 + 水泥墙），现在这套"场景"（黄铜吊灯 / 鎏金王座 / 烟酒钱 / 立柱幕布）
全部变成可解锁商品。**寒酸默认版是程序化几何**（`BareBulb` / `PlainChair` / `TinAshtray`），
所以商城不依赖任何资产管线就能有真实对比。

### 双货币，故意分开

| | 筹码 chips | 金印 seals |
|---|---|---|
| 来源 | 赢牌 / 救济金 | 赢一场 +30、弑君 +100（每次） |
| 用途 | 只用于对局买入 | 只用于当铺 |
| 会输掉吗 | **会** | **永不** |

分开的理由是付费信任：玩家挣来/买来的外观，不该因为一把牌就没了。以后真要接支付，
这条界线也让合规叙事干净得多。**商城只卖外观，一张牌的输赢都不会变——这条别破。**

### 防作弊

- 购买/装备**全部服务端校验**：客户端只发 id，价格和拥有权服务器说了算；扣款+发货在一个事务里；
  装备时重新验证拥有权（否则谁都能穿任何东西）。
- **PvP 金印服务端权威**（server 看过每一手，胜负和弑君数是事实）。
- **AI 局金印有界**：胜利奖励只在（已被 2× 钳制的）结算显示盈利时发放；弑君数按 Bo7 物理上限
  `MAX_REGICIDES_PER_MATCH = 7` 钳制，谎报也拿不到更多。

### 预览

`/shop` 左边就是你的房间，点商品**立刻换上给你看**——试穿纯本地，不花钱不请求服务器。
`TableScene` 接 `look` prop，`normalizeLoadout` 保证旧存档/已下架物品永远不会渲染出空洞。
