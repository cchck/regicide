'use client';

import { useEffect, useRef, useState } from 'react';

// Real anatomical ear illustration (helix, antihelix fold, tragus, concha, lobule all
// present as distinct internal contours) — sourced from a medical icon set rather than
// a flat minimalist UI glyph, so it reads as an actual ear instead of a cartoon outline.
const EAR_PATH =
  'M12.154 18c0-6.075 5.304-11 11.846-11s11.846 4.925 11.846 11c0 3.807-2.082 7.163-5.25 9.139c-2.689 1.676-5.52 4.315-5.52 7.772V35c0 3.314-2.892 6-6.46 6c-3.57 0-6.462-2.686-6.462-6v-2H10v2c0 4.418 3.857 8 8.615 8s8.616-3.582 8.616-8v-.089c0-2.404 2.013-4.52 4.565-6.112C35.536 26.467 38 22.501 38 18c0-7.18-6.268-13-14-13s-14 5.82-14 13zM23 14c-1.948 0-3.528 1.315-3.911 3.02h.028c.813.042 1.918.158 3.038.464c1.111.304 2.312.816 3.247 1.697c.964.909 1.598 2.169 1.598 3.827q0 .045-.004.09c-.146 1.61-.54 2.84-1.137 3.762c-.606.937-1.386 1.503-2.202 1.824c-.798.314-1.598.382-2.249.382c-.327 0-.629-.018-.878-.033l-.099-.007l-.3-.018l-.041.077a6 6 0 0 0-.277.674c-.064.174-.137.374-.233.614c-.21.525-.511 1.178-1.017 1.7c-.544.559-1.284.927-2.252.927c-.838 0-1.532-.192-2.076-.565a2.74 2.74 0 0 1-1.031-1.34A3.5 3.5 0 0 1 13 29.891a4 4 0 0 1 .04-.546l.003-.014l.001-.005v-.003c0-.001 0-.002.984.178l.984.179v-.003l.001-.003v-.003v.003l-.004.038a1.7 1.7 0 0 0 .073.695a.74.74 0 0 0 .281.376c.14.096.417.216.948.216c.402 0 .632-.131.817-.321c.221-.228.405-.574.595-1.049c.046-.115.097-.253.152-.401c.13-.35.282-.76.436-1.058q.05-.1.12-.213c.556-1.423 1.417-3.915 1.069-4.958c-.233-.698-.682-1.396-1.096-1.943c-.504-.422-.888-.83-1.12-1.318c-.286-.604-.285-1.235-.284-1.815v-.103c0-3.248 2.721-5.82 6-5.82s6 2.572 6 5.82h-2c0-2.075-1.756-3.82-4-3.82';

interface EarTrackerProps {
  won: number;
  color: 'blood' | 'teal';
  flip?: boolean;
}

const BAND_START = 10;
const BAND_HEIGHT = 7; // 4 bands * 7 = 28, matching the icon's content bounds (y: 10-38 on a 48-unit canvas)

export default function EarTracker({ won, color, flip = false }: EarTrackerProps) {
  const [flashIndex, setFlashIndex] = useState<number | null>(null);
  const prevWon = useRef(won);

  useEffect(() => {
    if (won > prevWon.current) {
      const idx = won - 1;
      setFlashIndex(idx);
      const t = setTimeout(() => setFlashIndex(null), 750);
      prevWon.current = won;
      return () => clearTimeout(t);
    }
    prevWon.current = won;
  }, [won]);

  const fill = color === 'blood' ? '#dd2222' : '#2a8a8a';
  const dim = color === 'blood' ? 'rgba(170,17,17,0.5)' : 'rgba(42,138,138,0.5)';
  const clipId = 'ear-clip-' + color + (flip ? '-f' : '');

  return (
    <div className={'relative w-14 h-14 sm:w-16 sm:h-16' + (flashIndex !== null ? ' ear-impact' : '')}>
      <svg viewBox="0 0 48 48" className="w-full h-full" style={{ transform: flip ? 'scaleX(-1)' : undefined }}>
        <defs>
          <clipPath id={clipId}>
            <path d={EAR_PATH} clipRule="evenodd" />
          </clipPath>
        </defs>

        {/* Base — always-visible dim silhouette */}
        <path d={EAR_PATH} fill={dim} fillRule="evenodd" />

        {/* 4 win bands, clipped to the ear silhouette */}
        <g clipPath={`url(#${clipId})`}>
          {[0, 1, 2, 3].map((i) => {
            const filled = i < won;
            return (
              <rect
                key={i}
                x="0"
                y={BAND_START + i * BAND_HEIGHT}
                width="48"
                height={BAND_HEIGHT}
                fill={filled ? fill : 'transparent'}
                opacity={filled ? 0.92 : 0}
                className={flashIndex === i ? 'ear-band-flash' : ''}
              />
            );
          })}
        </g>

        {/* Slash flash on the newly-won band */}
        {flashIndex !== null && (
          <line
            x1="6"
            y1={BAND_START + flashIndex * BAND_HEIGHT + 1.5}
            x2="42"
            y2={BAND_START + flashIndex * BAND_HEIGHT + BAND_HEIGHT - 1.5}
            stroke="#ffffff"
            strokeWidth="1.5"
            strokeLinecap="round"
            className="ear-slash-line"
            style={{ filter: 'drop-shadow(0 0 6px rgba(255,255,255,0.95))' }}
          />
        )}
      </svg>
    </div>
  );
}
